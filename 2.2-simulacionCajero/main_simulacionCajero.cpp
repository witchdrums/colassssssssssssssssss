...
string randomClient();
int secondsToLeave();
void timer1(int,int,int, int,string);
Cola cola;
...
int main(){
	...
	timer1(0, 0, secondsToLeave(), 0,"");

	return 0;
}

void timer1(int seconds, int timer, int timer2, int clientCount, string cliente){
	if (seconds == 60){
			cout<<"\n--------------------------------\n";
	...
	cout<<"Final de la cola: "<<cola.getFinal()<<endl;
		...
		*/
		...
		//clientTime(rand() % 3 + 2);
		sleep(1);
		timer1(++seconds, --timer, --timer2, clientCount, cliente);
	}
}

string randomClient(){
	return names[rand() % 24 + 0];
}
